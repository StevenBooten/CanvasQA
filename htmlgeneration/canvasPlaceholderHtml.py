...
def generatePlaceholderHtml(myCanvas, canvasQa):
    canvasQa['issues']['placeholders'] = 0
    placeholderIssues = {}
    ...
    for pageId, pageData in canvasQa['pages'].items():
        ...
        canvasQa['issues']['placeholders'] += len(pageData['placeholders']) if pageData['placeholders'] is not None else 0
    ...
    for assessmentGroupId, assessmentGroupData in canvasQa['assignments'].items():
        for assessmentId, assessmentData in assessmentGroupData['assignments'].items():
            ...
            for questionId, quizData in assessmentData['quiz'].items():
                ...
                canvasQa['issues']['placeholders'] += len(quizData['placeholders']) if quizData['placeholders'] is not None else 0
    htmlPlaceholders = htmlPlaceholdersGenerate(placeholderIssues)
    
    return htmlPlaceholders, canvasQa

def htmlPlaceholdersGenerate(placeholderIssues):
    html = (
            Article([Class('message')],
                Div([Class('message-header')],
                    P([], Span([], 'Placeholders'),
                        Span([Class('tag is-info ml-6')], 
                            A([Onclick(f'sectionExpand("collapsible-placeholder-check");')], 'collapse/expand')
                        ),
                ...
                Div([Id('collapsible-placeholder-check'), Class('message-body is-collapsible')],
